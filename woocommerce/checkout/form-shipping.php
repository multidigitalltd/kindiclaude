...
<?php
// Order-notes fieldset (הערות למלקט + הערות למשלוח). WooCommerce core renders
// this in its own form-shipping.php; our override above replaces that file, so
// the block is restored here verbatim — otherwise the notes fields vanish.
?>
<div class="woocommerce-additional-fields">
	...
	<?php if ( apply_filters( 'woocommerce_enable_order_notes_field', 'yes' === get_option( 'woocommerce_enable_order_comments', 'yes' ) ) ) : ?>
		...
		<?php if ( ! WC()->cart->needs_shipping() || wc_ship_to_billing_address_only() ) : ?>
			...
			<h3><?php esc_html_e( 'מידע נוסף', 'kindi' ); ?></h3>

		<?php endif; ?>

		<div class="woocommerce-additional-fields__field-wrapper">
			<?php foreach ( $checkout->get_checkout_fields( 'order' ) as $kindi_key => $kindi_field ) : ?>
				<?php woocommerce_form_field( $kindi_key, $kindi_field, $checkout->get_value( $kindi_key ) ); ?>
			<?php endforeach; ?>
		</div>
